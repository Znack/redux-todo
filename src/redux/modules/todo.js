...
export const actions = {
  ...
  receiveUsers: createAction(RECEIVE_USERS, (users) => {
    ...
    throw new Error('Expected users to be Array, but got: ${users}');
  }),
  ...
  receiveTodos: createAction(RECEIVE_TODOS, (todos) => {
    ...
    throw new Error('Expected todos to be Array, but got: ${todos}');
  }),
...
export const changeUser = (state, { payload }) => {
  if (state.users.filter((user) => user.id === payload).length) {
    return {...state, currentUser: payload};
  }
  throw new Error('There is no such user ${payload} in the state.todo.users');
};
